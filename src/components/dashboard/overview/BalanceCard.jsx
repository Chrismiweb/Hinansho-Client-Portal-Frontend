...
export default function BalanceCard() {
  return (
    <div className="bg-white border-2 border-[#F1F5F9] rounded-3xl p-6 shadow-sm">
      <p className="text-[18px] md:text-[20px] text-[#62748E] mb-1">Total Balance</p>

      <div className=" gap-3 mb-4">
        <h2 className="text-[37px] md:text-[40px] lg:text-[50px] font-bold">$689,372.00</h2>
  ...
  return (
    ...
    >
      <div className="flex w-full items-center justify-between md:hidden">
        ...
        {badge && (
            <p className="text-[14px] mt-2 md:hidden inline-block rounded-xl px-3 py-1.25 bg-[#FEF2F21A] text-red-400">{badge}</p>
          )}

          {subtitle && (
            <p className="text-[14px] mt-2 md:hidden inline-block px-3 py-1.25 rounded-xl bg-[#F8FAFC] text-[#90A1B9]">{subtitle}</p>
          )}
      </div>
      ...
      {badge && (
        <p className="text-[14px] mt-2 hidden md:inline-block rounded-xl px-3 py-1.25 bg-[#FEF2F21A] text-red-400">{badge}</p>
      )}

      {subtitle && (
        <p className="text-[14px] mt-2 hidden md:inline-block px-3 py-1.25 rounded-xl bg-[#F8FAFC] text-[#90A1B9]">{subtitle}</p>
      )}
    </div>
  );
}
