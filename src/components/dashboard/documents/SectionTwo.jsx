...
export default function SectionTwo({ documents = [], onPreview = () => {} }) {
  const [activeTab, setActiveTab] = useState("All");

  const filtered = activeTab === "All" ? documents : documents.filter(d => (d.documentType || '').toLowerCase() === activeTab.toLowerCase());
  ...
  return (
    ...
    <div className="bg-white rounded-3xl p-6 border border-[#E2E8F0] w-full">
      ...
      <div className="space-y-4">
        {filtered.length > 0 ? (
          filtered.map((doc) => (
            <DocumentItem key={doc._id} doc={doc} onPreview={() => onPreview(doc)} />
          ))
        ) : (
